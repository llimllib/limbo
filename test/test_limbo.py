...
def test_extra_plugin_two_success():
    hooks = limbo.init_plugins(
        ["test/plugins", "test/extraplugins", "test/moreextraplugins"], None
    )
    assert len(hooks) == 11
    assert "message" in hooks
    assert isinstance(hooks, dict)
...
def test_run_hook():
    hooks = limbo.init_plugins(["test/plugins"])
    assert limbo.run_hook(hooks, "message", {"text": "!echo bananas"}, None) == [
        "!echo bananas"
    ]
...
def test_handle_member_joined():
    event = {"type": "member_joined_channel", "user": "2"}
...
def test_handle_member_left():
    event = {"type": "member_left_channel", "user": "2"}
